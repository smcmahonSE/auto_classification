...
def parse_args():
    ...
    parser.add_argument(
        "--output-dir",
        default=f"artifacts/model/runs/{default_run_id}",
        help="Run-specific output directory. Defaults to a unique timestamped path.",
    )
    ...
    parser.add_argument(
        ...
        help="JSONL model registry with one immutable record per training run.",
    )
    parser.add_argument(
        "--latest-model-path",
        default="artifacts/model/latest/product_classifier.joblib",
        help="Convenience pointer updated each run for inference defaults.",
    )
    parser.add_argument(
        "--run-id",
        default=None,
        help="Optional stable run identifier. Defaults to UTC timestamp.",
    )
    parser.add_argument(
        "--pca-components",
        type=int,
...
def append_registry_record(path: Path, record: dict) -> None:
    path.parent.mkdir(parents=True, exist_ok=True)
    with path.open("a", encoding="utf-8") as f:
        f.write(json.dumps(record) + "\n")
...
def main():
    ...
    latest_model_path = Path(args.latest_model_path)
    latest_model_path.parent.mkdir(parents=True, exist_ok=True)
    shutil.copy2(joblib_path, latest_model_path)
    print(f"Updated latest model pointer copy: {latest_model_path}")
    ...
    registry_record = {
        ...
        "features_path": str(Path(args.features_path)),
        "output_dir": str(output_dir),
        "model_artifact_path": str(joblib_path),
        ...
        "artifact_sizes_mb": size_report,
    }
    registry_path = Path(args.registry_path)
    append_registry_record(registry_path, registry_record)
    print(f"Appended model registry record to {registry_path}")
